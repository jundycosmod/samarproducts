...
if(isset($_GET['action']) && $_GET['action'] == "delete"){
	$deleteSQL = sprintf("DELETE FROM cart WHERE cart_id=%s",
                       $_GET['cart_id']);
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	...
	foreach($_POST['cart_id'] as $cart_id){
	 $updateSQL = sprintf("UPDATE cart SET quantity=%s WHERE cart_id=%s",
                       $count[$x],
                       $id[$x]);
...
$products_incart = mysql_query($query_products_incart, $akonsudoy) or die(mysql_error());
$row_products_incart = mysql_fetch_assoc($products_incart);
$totalRows_products_incart = mysql_num_rows($products_incart);
$id[$i] = $row_products_incart['product_id'];
  ...
  mysql_free_result($products_incart);
  ?>
    </table>
  </div>
  <br />
...
$message = mysql_query($query_message, $akonsudoy) or die(mysql_error());
...
$total = mysql_query($query_total, $akonsudoy) or die(mysql_error());
