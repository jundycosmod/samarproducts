...
if (!function_exists("GetSQLValueString")) {
    ...
    $change_pass = mysqli_query($akonsudoy, $query_change_pass) or die(mysqli_error($akonsudoy));
    $row_change_pass = mysqli_fetch_assoc($change_pass);
    $totalRows_change_pass = mysqli_num_rows($change_pass);
    ...
    if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1") && ($_POST['oldpass'] == $row_change_pass['password']) && ($_POST['newpass'] == $_POST['conpass'])) {
        $updateSQL = sprintf("UPDATE users SET email=%s, password=%s WHERE user_id=%s", GetSQLValueString($_POST['email'], "text"), GetSQLValueString($_POST['newpass'], "text"), GetSQLValueString($_POST['user_id'], "int"));
...
<?php
mysqli_free_result($change_pass);
