...
if (isset($_GET['action']) && $_GET['action'] == "add") {
	...
	$row_cart = mysqli_fetch_assoc($akonsudoy, $cart);
	$totalRows_cart = mysqli_num_rows($akonsudoy, $cart);
	mysqli_select_db($akonsudoy, $database_akonsudoy);
	$query_check_quantity = "SELECT * FROM cart WHERE user_id = '" . $_SESSION['user_id'] . "' AND product_id = '" . $_GET['product_id'] . "'";
	$check_quantity = mysqli_query($akonsudoy, $query_check_quantity) or die(mysqli_error());
	$row_check_quantity = mysqli_fetch_assoc($akonsudoy, $check_quantity);
	$totalRows_check_quantity = mysqli_num_rows($akonsudoy, $check_quantity);
	...
	if ($_GET['product_id'] == $row_check_quantity['product_id']) {
		if ($totalRows_check_quantity == 0) {
			...
			//mysql_select_db($database_akonsudoy, $akonsudoy);
			$Result2 = mysqli_query($akonsudoy, $updateSQL) or die(mysqli_error());
		}
	} else {
		if ($totalRows_check_quantity == 0) {
			$row_check_quantity['quantity'] = $row_check_quantity['quantity'] + 1;
...
$products = mysqli_query($akonsudoy, $query_limit_products) or die(mysqli_error());
