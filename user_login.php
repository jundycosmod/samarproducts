...
if (isset($_POST['email']) && isset($_POST['button_Login'])) {
    ...
    $LoginRS__query = sprintf("SELECT email, password, access_level FROM users WHERE email=%s AND password=%s", GetSQLValueString($loginUsername, "text"), GetSQLValueString($password, "text"));

    $LoginRS = mysqli_query($akonsudoy, $LoginRS__query) or die(mysqli_error($akonsudoy));
    $loginFoundUser = mysqli_num_rows($LoginRS);
    if ($loginFoundUser) {

        $loginStrGroup = mysqli_fetch_assoc($LoginRS);
        $loginStrGroup = $loginStrGroup['access_level'];
        $query_user_id = "SELECT * FROM users WHERE email='" . $loginUsername . "'";
        $user_id = mysqli_query($akonsudoy, $query_user_id) or die(mysqli_error($akonsudoy));
